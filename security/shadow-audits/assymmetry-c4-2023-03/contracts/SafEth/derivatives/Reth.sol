...
contract Reth is IDerivative, Initializable, OwnableUpgradeable {
    ...
    */
     ...
     */
    function swapExactInputSingleHop(   // @audit-issue no deadline
        address _tokenIn,
        address _tokenOut,      
        uint24 _poolFee,
        uint256 _amountIn,
        uint256 _minOut
    ) private returns (uint256 amountOut) {         // private modifier so no-issue
    ...
    function deposit() external payable onlyOwner returns (uint256) {
        ...
        if (!poolCanDeposit(msg.value)) {
            ...
            uint256 minOut = ((((rethPerEth * msg.value) / 10 ** 18) *      //  precession loss
    ...
    function ethPerDerivative(uint256 _amount) public view returns (uint256) {
        ...
        else return (poolPrice() * 10 ** 18) / (10 ** 18);   //@audit-issue this was not uint256
    }
    ...
    function poolPrice() private view returns (uint256) {
        ...
        (uint160 sqrtPriceX96, , , , , , ) = pool.slot0();          // @audit-issue https://solodit.xyz/#issue-h-4-no-slippage-protection-during-repayment-due-to-dynamic-slippage-params-and-easily-influenced-slot0 On-chain slippage calculation using price from pool.slot0 can be easily manipulated causing users to receive less tokens than they intended. flash loans
        return (sqrtPriceX96 * (uint(sqrtPriceX96)) * (1e18)) >> (96 * 2);  // @audit-issue sqrtPriceX96 should be uint, overflow
    }
