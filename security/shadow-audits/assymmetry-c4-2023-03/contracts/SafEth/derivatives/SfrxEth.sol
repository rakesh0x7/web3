...
contract SfrxEth is IDerivative, Initializable, OwnableUpgradeable {  // @audit-issue same issues as wsteth.sol
    ...
    */
    ...
    function withdraw(uint256 _amount) external onlyOwner {
        IsFrxEth(SFRX_ETH_ADDRESS).redeem(  //@audit-issue sender should be msg.sender?
            _amount,
            address(this),
            address(this)
        );
        ...
        uint256 minOut = (((ethPerDerivative(_amount) * _amount) / 10 ** 18) *  //@check-again withdraw they havne't used ether perderivative in wsteth
    ...
    function deposit() external payable onlyOwner returns (uint256) {
        ...
        frxETHMinterContract.submitAndDeposit{value: msg.value}(address(this)); //@audit-issue no return value check
    ...
    function ethPerDerivative(uint256 _amount) public view returns (uint256) { 
        ...
        return ((10 ** 18 * frxAmount) /
            IFrxEthEthPool(FRX_ETH_CRV_POOL_ADDRESS).price_oracle()); // as the withdraw function having (10**18)
    }
